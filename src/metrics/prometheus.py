...
ACTIVE_CONNECTIONS = Gauge(
    "biocurator_active_connections", "Number of active connections", registry=metrics_registry
)
...
MODEL_TOKENS = Counter(
    ...
    ["provider", "model", "type"],  # token_type: input/output
    registry=metrics_registry,
)
...
CACHE_HIT_RATIO = Gauge(
    "biocurator_cache_hit_ratio", "Cache hit ratio", ["cache_type"], registry=metrics_registry
)
...
def create_metrics_router() -> APIRouter:
    ...
    async def metrics_endpoint():
        ...
        if not settings.monitoring.prometheus_enabled:
            return Response(content="Metrics disabled", status_code=404, media_type="text/plain")
