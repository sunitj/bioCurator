...
class ConfigLoader:
    ...
    def _load_yaml_config(self, config_path: str | None = None) -> None:
    ...
    def _validate_critical_config(self) -> None:
        ...
        critical_checks = [
            ...
            (
                self._config.secret_key != "change-me-in-production"
                or self._config.app_mode.value == "development",
        ...
        if self._config.app_mode.value == "production":
            critical_checks.extend(
                [
                    (
                        self._config.models.provider == "cloud"
                        and (
                            self._config.models.openai_api_key
                            or self._config.models.anthropic_api_key
                        ),
    ...
    def reload_config(self, config_path: str | None = None) -> ConfigSchema:
...
try:
    ...
    # During testing or CI, use defaults
    if os.getenv("CI") or os.getenv("PYTEST_CURRENT_TEST"):
        print(f"Warning: Using default configuration due to: {e}", file=sys.stderr)
...
def reload_settings(config_path: str | None = None) -> ConfigSchema:
